...
import { AgyModelCatalog, readOption, type AgyOptionValue } from './model-catalog.js'
import { isEffortRejection, rememberEffortUnsupported, shouldPassEffort } from './effort.js'
...
export class AgyLlmAdapter extends LlmAdapter {
   ...
   */
  private readonly catalog: AgyModelCatalog
  ...
  constructor(
    ...
    super()
    this.catalog = new AgyModelCatalog({
      command: () => readOption(this.options.command),
      model: () => readOption(this.options.model),
      proxy: () => readOption(this.options.proxy),
    })
  }
  ...
  override async *stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    // 全部走 readOption():配置项可以是 getter,设置面板改动即时生效,不必重启。
    const command = readOption(this.options.command) ?? 'agy'
    // 请求级 model 优先(子代理可经 agentOptions.model 动态指定),回退到配置值。
    const model = options.model ?? readOption(this.options.model) ?? 'gemini-3.7-flash-high'
    const effort = readOption(this.options.effort) ?? 'high'
    ...
    // 代理解析(异步、带 60 秒缓存):auto/留空 = 探测系统代理(读不到就直连),
    // 手动值原样注入,off = 强制直连;不猜端口。关掉 TUN 后靠它把 agy 流量
    // 交给代理 —— agy 是 Go 程序,读取 HTTPS_PROXY。
    const proxyEnv = agyProxyEnv((await resolveAgyProxy(readOption(this.options.proxy))).url)
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const resumeArgs = attempt > 1 && conversationId !== undefined
          ...
          : []
        // --effort 是逐模型能力,不是全局开关:模型 id 自带档位后缀
        // (gemini-3.8-flash-high / gpt-oss-120b-medium)或属于不支持 effort 的
        // 家族(Claude 全系)时,传了会被 AGY 在参数校验阶段直接拒绝
        // ("invalid model selection ... --effort is not supported / conflicts")。
        // 判定与理由见 effort.ts。
        const effortArgs = shouldPassEffort(model, effort) ? ['--effort', effort] : []
        const proc: ChildProcess = spawn(command, [
          ...
          windowsHide: true,
          // AGY 资格检查/API 流量走代理解析结果;未探测到代理则保持直连。
          env: {
            ...process.env,
            ...proxyEnv ?? {},
          },
        ...
        try {
          ...
          closeLines = () => { rl.close() }
          // 中止可能发生在上面的 await(prompt 构建 / 代理解析)期间:那时 abort
          // 监听器还没注册,而 spawn 之后那次 killTree() 只销毁了 stdout ——
          // readline 的 async iterator 必须靠 rl.close() 才结束(实测)。少了这一步,
          // 用户在这段窗口里点停止,读循环会一直挂到 AGY 自己退出为止(表现为
          // "点了停止没反应")。
          if (options.signal?.aborted) killTree()
        ...
        // --effort 被 AGY 的参数校验拒绝(模型不支持 effort / 与模型自带档位冲突):
        // 这不是任务失败 —— 拒绝发生在会话启动之前(实测 0 token、无会话、无任何
        // 工具副作用),所以去掉 --effort 立刻重跑是安全的。同时记住该模型,后续
        // 调用不再传;attempt 减一抵消循环自增,即不消耗重试次数。
        // 只在还没有任何输出时降级重跑,避免把已产出的文本重复一遍。
        if (!hasOutput && resultError !== undefined && isEffortRejection(resultError)) {
          rememberEffortUnsupported(model)
          attempt -= 1
          continue
        }
  ...
  override providerInfo(provider: string): { id: string; name: string } {
    return { id: provider, name: 'Antigravity CLI (Gemini)' }
  }
  ...
  override listModels(provider: string): Promise<LlmResolvedModelInfo[]> {
    this.catalog.scheduleRefresh()
    return Promise.resolve(this.catalog.list(provider))
  }
  ...
  override resolveModel(provider: string, model: string, _signal?: AbortSignal): Promise<LlmResolvedModelInfo> {
    return Promise.resolve({
      provider,
      id: model,
      // 用目录里的显示名(Gemini 3.8 Flash (High) 之类),选型器里更好认。
      name: this.catalog.displayName(model),
