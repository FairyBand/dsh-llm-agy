...
import { readOption, type AgyOptionValue } from './model-catalog.js'
 ...
 */
export function parseAgyQuotaOutput(raw: string): AgyQuotaGroup[] {
 ...
 */
export function parseAgyQuotaText(text: string): AgyQuotaGroup[] {
  const groupMap = new Map<string, AgyQuotaBucket[]>()
 ...
 */
export function parseAgyCreditsOutput(raw: string): AgyCreditsInfo | undefined {
  const text = String(raw ?? '').trim()
...
async function execAgyPrint(command: string, slashCmd: string, proxy?: string): Promise<string> {
...
let cachedSummary: AgyQuotaSummary | undefined
let lastFetchTime = 0
let inFlightPromise: Promise<AgyQuotaSummary> | undefined
 ...
 */
export async function runAgyQuota(
  commandValue?: AgyOptionValue<string>,
  proxyValue?: AgyOptionValue<string>,
  forceRefresh = false,
): Promise<AgyQuotaSummary> {
  const now = Date.now()
 ...
 */
export function formatQuotaSummaryMarkdown(summary: AgyQuotaSummary): string {
  if (!summary.ok) {
    return `**AGY 账号用量查询失败**: ${summary.error ?? '未返回有效配额数据'}\n\n*提示: 请确认已在系统终端运行 \`agy\` 登录 Google 账号,并检查插件设置中的代理配置。*`
  }

  const lines: string[] = ['### AGY 当前账号剩余配额与用量']
