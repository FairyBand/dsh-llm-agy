...
export async function runAgyText(options: AgyRunTextOptions): Promise<string> {
  const passEffort = shouldPassEffort(options.model, options.effort)
  try {
    return await runAgyTextOnce({ ...options, effort: passEffort ? options.effort : undefined })
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    if (!passEffort || !isEffortRejection(message)) throw error
    rememberEffortUnsupported(options.model)
    return await runAgyTextOnce({ ...options, effort: undefined })
  }
}

/** 单次执行(不含 effort 兼容处理)。 */
async function runAgyTextOnce(options: AgyRunTextOptions): Promise<string> {
  const command = options.command
  // 是否需要 --effort 由 runAgyText 决定,这里只负责拼参。
  const effortArgs = options.effort !== undefined && options.effort.length > 0
    ? ['--effort', options.effort]
    : []
  // 代理解析(异步、带缓存):关掉 TUN 后靠注入 HTTPS_PROXY 让 agy 走 Clash;
  // 没有系统代理时保持直连,绝不猜端口。
  const proxyEnv = agyProxyEnv((await resolveAgyProxy(options.proxy)).url)
