...
/** 运行期可变的 AGY 配置(设置面板改动即时生效,不必重启)。 */
interface AgyRuntimeOptions {
  command: string
  model: string
  effort: string
  proxy: string
...
function registerSubagentTools(ctx: Context, getOptions: () => AgyRuntimeOptions): void {
  // 描述文本用注册时的快照;真正的模型查询走 getOptions() 实时读取。
  const model = getOptions().model
...
export function apply(ctx: Context, config: Config): void {
  ...
  // 传 getter 而不是值:设置面板里改命令/模型/代理即时生效,不必重启。
  // (原先在注册时一次性求值,面板改动后对话仍用启动快照 —— 这正是
  // "面板里测试 hi 正常、对话里却连不上"的典型成因。)
  ...
  if (config.registerSubagentTools !== false) {
    registerSubagentTools(ctx, () => agyOptions)
  }
  ...
  // 与主适配器共用同一份动态配置:之前这里读的是 config(启动快照),
  // 于是搜索/读图用的是 plugin config 的代理(常为空),与对话用的代理不一致 ——
  // 关掉 TUN 后表现为"对话能通、搜索/看图连不上"。
  const searchOptions = () => ({
    command: agyOptions.command,
    model: agyOptions.model,
    effort: agyOptions.effort,
