...
export const BUILTIN_AGY_MODELS = [
...
/** 内置表的 id → 显示名映射。 */
const BUILTIN_NAME_BY_ID = new Map(BUILTIN_AGY_MODELS);
/** 刷新结果的新鲜期:期内不重复跑 agy。 */
 ...
 * @returns 当前值。
 */
export function readOption(value) {
 ...
 */
export function parseAgyModelsOutput(out) {
    const seen = new Set();
    const models = [];
 ...
 */
export async function runAgyModels(command, proxy) {
...
export class AgyModelCatalog {
    ...
    scheduleRefresh() {
        if (this.refreshing !== undefined)
            return;
        if (this.refreshedAt !== 0 && Date.now() - this.refreshedAt < REFRESH_TTL_MS)
            return;
        if (this.scheduled)
            return;
        this.scheduled = true;
