...
/** 读取设置作用域的当前值(兼容 subscribe/getSnapshot 两种形态)。 */
     ...
     *   · next     —— 用户最近一次选择(在会话里换模型后,这里立刻就是新选择);
     *   · lastUsed —— 该会话真正发过请求的模型(历史对话的既有选择)。
     * 因此按 **next → lastUsed → 全局默认** 取值:切换会话与切换模型都能立即跟随
     * (顺序反了会出现"历史会话里换成 Gemini 徽标不出现、换回 DeepSeek 也不消失")。
    ...
    function useCurrentProvider(props) {
      ...
      if (typeof useProjection === 'function') {
        try {
          ...
          selection = next || lastUsed || undefined
        } catch { /* 投影不可用时回退 */ }
      }
      const provider = selection && typeof selection.provider === 'string' && selection.provider !== ''
        ? selection.provider
        : readDefaultProvider(c)
      const model = selection && typeof selection.model === 'string' ? selection.model : undefined
    ...
    /**
     * 从模型 id 判断它属于 AGY 的哪个配额组。
     * AGY 把 Gemini 与 Claude/GPT 的额度分开计,所以徽标要按当前模型选组。
     * @returns 'gemini' | 'claude' | 'gpt' | undefined
     */
    function modelQuotaKind(modelId) {
      if (typeof modelId !== 'string' || modelId === '') return undefined
      const id = modelId.toLowerCase()
      if (id.includes('gemini')) return 'gemini'
      if (id.includes('claude')) return 'claude'
      if (id.includes('gpt') || id.includes('oss')) return 'gpt'
      return undefined
    }

    /** 在配额组列表里找与关键字匹配的组。 */
    function findGroup(groups, keyword) {
      return groups.find((g) => typeof g.name === 'string' && g.name.toLowerCase().includes(keyword))
    }

    /**
     * 提取配额指标(用于徽标与详情展示)。
     * @param quota 服务端返回的配额汇总。
     * @param modelId 当前会话选中的模型 id,用于挑选对应的配额组。
     */
    function extractQuotaSummary(quota, modelId) {
      if (!quota || !quota.groups || quota.groups.length === 0) {
        return null
      }
      const groups = quota.groups
      const kind = modelQuotaKind(modelId)
      // 按当前模型所属的组选:gemini → Gemini 组;claude/gpt → Claude/GPT 组。
      // 认不出模型或该组缺失时退回第一个组(通常是 Gemini),避免徽标空白。
      let group
      if (kind === 'gemini') group = findGroup(groups, 'gemini')
      else if (kind === 'claude') group = findGroup(groups, 'claude') ?? findGroup(groups, 'gpt')
      else if (kind === 'gpt') group = findGroup(groups, 'gpt') ?? findGroup(groups, 'claude')
      if (group === undefined) group = groups[0]

      const bucketName = (b) => (typeof b.name === 'string' ? b.name.toLowerCase() : '')
      const isFiveHour = (b) => b.window === '5h' || bucketName(b).includes('5-hour') || bucketName(b).includes('five')
      const isWeekly = (b) => b.window === 'weekly' || bucketName(b).includes('weekly')
      const pctOf = (b) => (b === undefined || b === null
        ? null
        : (typeof b.percentage === 'number' ? b.percentage : Math.round((b.remainingFraction ?? 1) * 100)))

      const b5h = group.buckets?.find(isFiveHour)
      const bWeek = group.buckets?.find(isWeekly)
      const pct5h = pctOf(b5h)
      const pctWeek = pctOf(bWeek)
    ...
    function QuotaInputBadge(props) {
      ...
      const { provider, model } = useCurrentProvider(props)
      const showQuota = isAgyProvider(provider)
      const { data: quota, loading, error, refresh } = useAgyQuota(showQuota, props && props.ctx)
      // 按当前模型所属的配额组(Gemini / Claude+GPT)取数
      const summary = extractQuotaSummary(quota, model)

      // 只在主模型由 Antigravity(AGY)provider 提供时展示紧凑徽标。
      if (!showQuota) return null

      const groupLabel = summary?.groupName ?? 'AGY'
      let badgeText = `⚡ ${groupLabel}`
      let dotColor = '#10b981'
      let titleText = `点击查看 ${groupLabel} 剩余用量`
      ...
      if (loading && !quota) {
        ...
        // 主数值 = 5 小时剩余额度;悬停显示该组的 5h/每周明细
        badgeText = `⚡ ${summary.pct5h ?? summary.mainPct}%`
        dotColor = summary.statusColor
        titleText = `${groupLabel}: 5小时剩余 ${summary.pct5h ?? '-'}% · 每周剩余 ${summary.pctWeek ?? '-'}%(点击查看详情)`
      } else if (error) {
        badgeText = '⚡ 失败'
        dotColor = '#ef4444'
