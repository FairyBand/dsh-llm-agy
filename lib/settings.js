...
export const AgySettingsConfig = z.object({
    ...
    effort: z.string().default('high').description('推理强度 low/medium/high。AGY 的档位由模型 id 决定(gemini-3.8-flash-high 等自带后缀),Claude/GPT-OSS 则完全不支持 --effort;这些情况下插件会自动省略该参数(否则 AGY 直接拒绝),仅对真正支持 --effort 的模型生效'),
 ...
 * @returns 校验结果;结果进缓存。
 */
export function agyInstalledAsync(command) {
                    ...
                    try {
                        ...
                        if (quota.ok && quota.groups.length > 0) {
                            ...
                            if (b5h || bWeek) {
                                const parts = [];
                                if (b5h)
                                    parts.push(`5h ${b5h.percentage}%`);
                                if (bWeek)
                                    parts.push(`周 ${bWeek.percentage}%`);
