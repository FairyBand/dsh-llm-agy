 ...
 */
export function shouldPassEffort(model, effort) {
    ...
    if (id === '')
        return true;
    if (EFFORT_TIER_SUFFIX_RE.test(id))
        return false;
    if (EFFORT_UNSUPPORTED_FAMILY_RE.test(id))
        return false;
 ...
 */
export function rememberEffortUnsupported(model) {
    const id = (model ?? '').trim();
    if (id !== '')
        learnedNoEffort.add(id);
}
/**
 * 该模型是否已被记为不支持 `--effort`(诊断/测试用)。
 * @param model 模型 id。
 */
export function isEffortUnsupported(model) {
    const id = (model ?? '').trim();
...
export function resetEffortSupportCache() {
