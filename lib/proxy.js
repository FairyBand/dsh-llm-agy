...
function normalizeProxyServer(value) {
    ...
    for (const scheme of ['https', 'http', '']) {
        ...
        if (/^socks/i.test(entry))
            continue; // 纯 socks 条目跳过,避免误当 HTTP 代理
 ...
 */
function queryOutput(command, args) {
 ...
 */
async function readOsSystemProxy() {
...
async function detectSystemProxy() {
    ...
    if (fromEnv !== undefined) {
        ...
        if (target !== undefined)
            return { url: target.url, source: 'system-env' };
    }
    const fromOs = await readOsSystemProxy();
    if (fromOs !== undefined)
        return { url: fromOs.url, source: 'system-os' };
    return undefined;
}
/** 缓存:配置值 → 解析结果。 */
const cache = new Map();
...
async function detect(spec) {
    ...
    if (spec !== '' && !AUTO_WORDS.has(lower)) {
        ...
        if (target !== undefined)
            return { url: target.url, source: 'configured' };
 ...
 */
export async function resolveAgyProxy(spec) {
    const key = (spec ?? '').trim();
 ...
 */
export function agyProxyEnv(url) {
