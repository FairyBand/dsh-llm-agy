 ...
 * 便捷:解析并直接给出 env 片段。
 * @param spec 代理配置值。
 * @returns `{ env, url, source }`。
 */
export async function resolveAgyProxyEnv(spec) {
  const resolved = await resolveAgyProxy(spec);
  return { env: agyProxyEnv(resolved.url), url: resolved.url, source: resolved.source };
}
