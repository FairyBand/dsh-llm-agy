...
/** 注册配额查询工具。 */
export function registerAgyQuotaTool(ctx, options) {
    const toolName = options.toolName ?? 'check_agy_quota';
    ctx.tools.register(defineTool({
        name: toolName,
        description: 'Check the remaining quota, 5-hour/weekly usage limits, and AI credit balance for the currently logged-in '
