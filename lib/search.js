...
export class AgySearchProvider {
    ...
    verifyAvailable() {
        ...
        try {
            ...
            proc.on('error', () => { clearTimeout(timer); this.availableCache = false; this.verifying = false; });
            proc.on('close', (code) => { clearTimeout(timer); this.availableCache = code === 0; this.verifying = false; });
        }
        catch {
